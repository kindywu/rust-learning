...
impl<'a> Expr<'a> {
    ...
    pub fn eval(&mut self) -> Result<i32> {
        let result = self.compute_expr(1, 0)?;
    ...
    // 计算单个 Token或者子表达式
    fn compute_atom(&mut self, level: usize) -> Result<i32> {
        match self.iter.peek() {
            ...
            Some(Token::LeftParen) => {
                self.iter.next();
                let result = self.compute_expr(1, level + 1)?;
    ...
    // 92 + 5 ^ 2 + 5 * 27 - 24 / ( 1 + 3 )
    fn compute_expr(&mut self, min_prec: i32, level: usize) -> Result<i32> {
        // 计算第一个 Token
        let mut atom_lhs = self.compute_atom(level)?;

        let spaces = " ".repeat(level * 10);
        ...
        loop {
            ...
            match token.compute(atom_lhs, atom_rhs) {
                Some(res) => {
                    atom_lhs = {
                        println!("{spaces}compute:{atom_lhs}{token}{atom_rhs}={res}");
                        println!("{spaces}atom_lhs={res}");
                        res
                    }
                }
